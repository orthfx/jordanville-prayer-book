...
function extractSections(lines: string[], toc: Array<{ title: string; page: number }>): Section[] {
  ...
  for (let i = 0; i < toc.length; i++) {
    ...
    for (let j = contentStartIdx; j < lines.length; j++) {
      ...
      if (normalizedLine === normalizedCurrent ||
          normalizedLine.startsWith(normalizedCurrent)) {
        sectionStart = j
        break
      }
    }
    ...
    if (nextEntry) {
      ...
      for (let j = sectionStart + 1; j < lines.length; j++) {
        ...
        if (normalizedLine === normalizedNext ||
            normalizedLine.startsWith(normalizedNext)) {
          sectionEnd = j
          break
        }
      }
...
function main() {
  ...
  fs.writeFileSync(
    path.join(outputDir, 'parsed-prayerbook.json'),
